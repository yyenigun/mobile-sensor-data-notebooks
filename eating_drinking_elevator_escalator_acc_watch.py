...
# Loading Accelerometer Data from SmartWatch

msWatchEatingPath = r'/Users/yalcin.yenigun/dev/workspaces/gsu/Sample Dataset/Eating-Drinking/csv'
msWatchEatingFiles = glob.glob(msWatchEatingPath + "/0_MsBandAccelerometer*.csv")
accMsDfEating = pd.DataFrame()
list_ = []
for file_ in msWatchEatingFiles:
    df = pd.read_csv(file_, index_col=None, header=0)
    list_.append(df)
accMsDfEating = pd.concat(list_)
accMsDfEating['label'] = 'eating'

msWatchElevatorPath = r'/Users/yalcin.yenigun/dev/workspaces/gsu/Sample Dataset/Elevator-Escalator/csv-segments/dir_Elevator*'
msWatchElevatorFiles = glob.glob(msWatchElevatorPath + "/trim_0_MsBandAccelerometer*.csv")
accMsDfElevator = pd.DataFrame()
list_ = []
for file_ in msWatchElevatorFiles:
    df = pd.read_csv(file_, index_col=None, header=0)
    list_.append(df)
accMsDfElevator = pd.concat(list_)
accMsDfElevator['label'] = 'elevator'

msWatchEscalatorPath = r'/Users/yalcin.yenigun/dev/workspaces/gsu/Sample Dataset/Elevator-Escalator/csv-segments/dir_Escalator*'
msWatchEscalatorFiles = glob.glob(msWatchEscalatorPath + "/trim_0_MsBandAccelerometer*.csv")
accMsDfEscalator = pd.DataFrame()
list_ = []
for file_ in msWatchEscalatorFiles:
    df = pd.read_csv(file_, index_col=None, header=0)
    list_.append(df)
accMsDfEscalator = pd.concat(list_)
accMsDfEscalator['label'] = 'escalator'

accDfFrames = [accDfEating, accDfElevator, accDfEscalator, accMsDfEating, accMsDfElevator, accMsDfEscalator]
...
random_forest_model = h2o.H2ORandomForestEstimator(
    model_id="EatingDrinkingElevatorEscalatorAccWatch",
    ntrees=20,
    max_depth=10,
    min_rows=4,
    nfolds=10,
